...
if __name__ == '__main__':
    ...
    if order == '0':
        ...
        config = Config('../config/visual/visual.conf')
        Display(config).render()
        exit(0)

    algorithms = {'1': 'UserKNN', '2': 'ItemKNN', '3': 'BasicMF', '4': 'SlopeOne', '5': 'SVD', '6': 'PMF',
    ...
    try:
        config = Config('../config/'+algorithms[order]+'.conf')
    except KeyError:
        print 'Error num!'
        exit(-1)
    recSys = RecQ(config)
