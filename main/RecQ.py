...
class RecQ(object):
    ...
    def __init__(self, config):
        self.trainingData = [] # [user, item, rating] training data
        self.testData = [] # [user, item, rating] test data
        self.relation = [] # [head user, tail user, weight] social relation data
        self.measure = []
        self.config = config
        ...
        if self.config.contains('evaluation.setup'):
            ...
            if self.evaluation.contains('-b'):
                binarized = True
                threshold = float(self.evaluation['-b'])
            if self.evaluation.contains('-testSet'):
                # specify testSet
                self.trainingData = FileIO.loadDataSet(config, binarized=binarized, threshold=threshold)
                self.testData = FileIO.loadDataSet(config, self.evaluation['-testSet'], bTest=True,binarized=binarized,threshold=threshold)
            elif self.evaluation.contains('-ap'):
                # auto partition
                self.trainingData = FileIO.loadDataSet(config, binarized=binarized, threshold=threshold)
                self.trainingData, self.testData = DataSplit.dataSplit(self.trainingData, test_ratio=float(self.evaluation['-ap']))
    ...
    def execute(self):
        ...
        if self.evaluation.contains('-cv'):
            ...
            # create instance of algorithm and run method derived from father class
            eval(recommender).execute()


def run(measure, algorithm, order):
    measure[order] = algorithm.execute()